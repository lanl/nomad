...
5. The model's {py:meth}`TorchModuleTool.from_pretrained <nomad.fm_base_tool.TorchModuleTool.from_pretrained>` SHALL do the following:
   - Accept an absolute model-directory path as its first positional argument and
     require no additional arguments. Implementations MAY accept optional keyword arguments.
...
3. If local model directories are used, names SHALL satisfy [SEP-986][] tool naming constraints.
...
5. A single `TorchModuleTool` instance SHALL use only `TorchModuleTool.device`;
